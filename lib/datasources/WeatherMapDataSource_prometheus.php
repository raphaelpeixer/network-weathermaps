...
class WeatherMapDataSource_prometheus extends WeatherMapDataSource {
    ...
    function ReadData($targetstring, &$map, &$item)
    {
        ...
        if(preg_match($this->prom_regex_dual_value,$targetstring,$matches))
        {
            ...
            $in_query  = 'ports_ifInOctets_rate{ifName="' . $intf . '",hostname="' . $instance . '"}*8';
            $out_query  = 'ports_ifOutOctets_rate{ifName="' . $intf . '",hostname="' . $instance . '"}*8';

            $timestamp = strval(file_get_contents("/var/www/html/Weathermap/lib/timestamp.txt"));
            $timestamp = substr($timestamp,0, -1);

            if($timestamp != "&time=now")
            {
                // IN
                $query = $in_query . $timestamp;
                $call = $url . $query;
                $inbw = $this->GetPromData($call);
                // OUT
                $query = $out_query . $timestamp;
                $call = $url . $query . $timestamp;
